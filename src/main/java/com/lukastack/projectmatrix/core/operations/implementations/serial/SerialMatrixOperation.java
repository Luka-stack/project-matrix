package com.lukastack.projectmatrix.core.operations.implementations.serial;

import com.lukastack.projectmatrix.core.equations.SerialElementWiseMatrixEquation;
import com.lukastack.projectmatrix.core.matrices.CreateMatrix;
import com.lukastack.projectmatrix.core.matrices.Matrix;

public abstract class SerialMatrixOperation implements SerialElementWiseMatrixEquation, CreateMatrix {

    protected final Class<? extends Matrix> clazz;

    protected SerialMatrixOperation(Class<? extends Matrix> clazz) {
        this.clazz = clazz;
    }

    protected Matrix createMatrix(int rows, int cols) {

        return this.createMatrix(clazz, rows, cols);
    }
}
